...
void main(){
  ...
  lightdir = vec3(10,5,0);//LightPosition_cameraspace + EyeDirection_cameraspace;
...
void main() {
    ...
    vec2 q = pos.xz / vec2(20.0f, 15.0f);
    vec2 p = -1.0 + 2.0 * q;
    vec2 qq;
    vec2 r;
    float time = 2485.0f;
    ...
    color *= 3.5;
    color /= 5.0;
    vec3 c = vec3(0.6f-color,0.6f-color,0.6f-color);

    vec3 n = normalize( normal );
    vec3 l = normalize( lightdir );
    float cost = clamp( dot( n,l ), 0,1 );
    float ambient = 0.15f;
    vec4 color2 = vec4(vec3(ambient) + c * cost,1.0f);
