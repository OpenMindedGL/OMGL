...
void main(){
  ...
  //v_normal = (vec4(getnormals(u_HeightMapLinear, pp).xzy,0.0f)).xyz;
  v_normal = getnormals(u_DefaultSampler , pp).xzy;
...
void main(){
  ...
  //vec3 v_normal = (u_V * vec4(getnormals(u_DefaultSampler , pp).xzy,0.0f)).xyz;
  vec3 n = normalize( (u_V * vec4(v_normal,0)).xyz );

  vec3 lightPos = (u_V * vec4(0.0f,10000.0f,0.0f,1)).xyz;
  vec3 to_light = lightPos;
