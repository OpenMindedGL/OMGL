...
Sky::Sky(glm::vec2 center, YGen* rectoNoise, YGen* versoNoise){
  ...
  compute_indices_sky();
  compute_normals();
  Init(GL_TRIANGLE_STRIP, "shaders/PerlinClouds.shader");
  //Init(GL_TRIANGLE_STRIP, "shaders/Clouds.shader");
}

void Sky::compute_normals(){
  ...
  for(unsigned int i = 0; i<m_Vertices.size();i++){
    if ((m_Vertices[i].pos).y < 1100.0f){
      m_Vertices[i].normal = normalize(m_Vertices[i].normal);
    }
  } 
}

void Sky::compute_indices_sky(){
...
void Sky::initload_sky(glm::vec2 center){
...
glm::i32vec2 Sky::GetChunk(glm::vec2 coords){
