...
int init(){
  ...
  for (int i = 0; i < 50600; i++){
    ...
    while (courbe2[m][1] <= g_vertex_buffer_data[(i*3) + 2])
      m++;
    GLfloat coef_dir = ((courbe1[n][0] - courbe1[n-1][0]) / (courbe1[n][1] - courbe1[n-1][1])); 
    GLfloat coef_dir2 = ((courbe2[m][0] - courbe2[m-1][0]) / (courbe2[m][1] - courbe2[m-1][1])); 
    g_vertex_buffer_data[(i*3) + 1] = 
      ((courbe1[n-1][0] - (coef_dir * courbe1[n-1][1])) + ((g_vertex_buffer_data[i*3]) * coef_dir)) * 
      ((courbe2[m-1][0] - (coef_dir2 * courbe2[m-1][1])) + ((g_vertex_buffer_data[(i*3) + 2]) * coef_dir2));
  }
