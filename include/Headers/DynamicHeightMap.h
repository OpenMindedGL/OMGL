...
public:
  ...
  DynamicHeightMap( NoiseGen* n, unsigned int width, float texsize, glm::vec2 step = glm::vec2(1.0f,1.0f), glm::i32vec2 base = glm::i32vec2(0,0)) : NoiseTexture(n,width,texsize,step,base), DynamicTexture(), HeightMap(n,width,texsize,step,base){ m_TorBegin = m_Base; m_TorBase = glm::i32vec2(0);}
