...
class Texture 
{
  protected :
    ...
    inline unsigned int LoadDDS(const std::string& path) { return LoadDDS(path, GL_TEXTURE_2D); }
    inline unsigned int LoadOther(const std::string& path) { return LoadOther(path, GL_TEXTURE_2D); }
    void GenDDS();
    void GenOther();
